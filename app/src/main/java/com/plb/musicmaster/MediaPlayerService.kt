...
class MediaPlayerService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        NotificationUtil.createChannel(this) // Create notification channel
        ...
        // Start foreground notification
        startForeground(NOTIFICATION_ID, NotificationUtil.foregroundNotification(this)).also {
            isForegroundService = true
        }
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        // Handle action passed from [MediaPlayerReceiver]
        when (SongAction.values()[intent?.action?.toInt() ?: SongAction.Nothing.ordinal]) {
            SongAction.Pause -> songController?.pause()
            SongAction.Resume -> songController?.play()
            SongAction.Stop -> songController?.stop()
